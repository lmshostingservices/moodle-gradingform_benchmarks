...
require_once($CFG->dirroot . '/grade/grading/form/benchmarks/checklisteditor.php');
MoodleQuickForm::registerElementType('checklisteditor', $CFG->dirroot.'/grade/grading/form/benchmarks/checklisteditor.php', 'MoodleQuickForm_checklisteditor');
...
class gradingform_benchmarks_editchecklist extends moodleform {
    ...
    public function definition() {
        ...
        // name
        $form->addElement('text', 'name', get_string('name', 'gradingform_benchmarks'), array('size'=>52));
        ...
        $form->addElement('select', 'status', get_string('checkliststatus', 'gradingform_benchmarks'), $choices)->freeze();
        ...
        $buttonarray = array();
        $buttonarray[] = &$form->createElement('submit', 'savechecklist', get_string('savechecklist', 'gradingform_benchmarks'));
        if ($this->_customdata['allowdraft']) {
            $buttonarray[] = &$form->createElement('submit', 'savechecklistdraft', get_string('savechecklistdraft', 'gradingform_benchmarks'));
        }
    ...
    public function definition_after_data() {
        ...
        if (!$el->getValue()) {
            ...
            if ($vals[0] == gradingform_controller::DEFINITION_STATUS_READY) {
                $this->findButton('savechecklist')->setValue(get_string('save', 'gradingform_benchmarks'));
            }
        }
    }
     ...
     *
     * @param gradingform_benchmarks_controller $controller
     * @return boolean
     */
    public function need_confirm_regrading($controller) {
        ...
        $el->setValue(get_string('backtoediting', 'gradingform_benchmarks'));
        $el->unfreeze();
