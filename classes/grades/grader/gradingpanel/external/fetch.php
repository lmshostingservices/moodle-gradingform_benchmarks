...
class fetch extends external_api {
    ...
    public static function execute(string $component, int $contextid, string $itemname, int $gradeduserid): array {
        ...
        if (BENCHMARKS !== $gradeitem->get_advanced_grading_method()) {
            throw new moodle_exception(
                "The {$itemname} item in {$component}/{$contextid} is not configured for advanced grading with benchmarks"
            );
        }
    ...
    public static function get_fetch_data(gradeitem $gradeitem, stdClass $gradeduser, bool $isgrading = true): array {
        ...
        return [
            'templatename' => 'gradingform_benchmarks/grades/grader/gradingpanel',
            'hasgrade' => $hasgrade,
            'grade' => [
