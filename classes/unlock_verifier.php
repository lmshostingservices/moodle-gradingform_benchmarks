...
class unlock_verifier {
    ...
    const CACHE_KEY = 'gradingform_benchmarks_unlocked';
    ...
    private static function get_credentials(): array {
        ...
        if (empty($siteid)) {
            $siteid = get_config('gradingform_benchmarks', 'siteid');
        }
        if (empty($apikey)) {
            $apikey = get_config('gradingform_benchmarks', 'apikey');
        }
    ...
    public static function show_unlock_notice(): void {
        $message = get_string('unlock_required', 'gradingform_benchmarks');
