...
if (!$controller->is_form_defined() || empty($options['alwaysshowdefinition'])) {
    throw new moodle_exception('nopermissions', 'error', '', get_string('previewchecklist', 'gradingform_benchmarks'));
}

$title = get_string('gradingof', 'gradingform_benchmarks', $manager->get_area_title());
$PAGE->set_url(new moodle_url('/grade/grading/form/benchmarks/preview.php', array('areaid' => $areaid)));
...
echo $OUTPUT->box($controller->get_formatted_description(), 'gradingform_benchmarks-description');
