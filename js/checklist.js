M.gradingform_benchmarks = M.gradingform_benchmarks || {};

/**
 * This function is called for each checklist on page.
 */
M.gradingform_benchmarks.init = function(Y, options) {
    Y.on('click', M.gradingform_benchmarks.itemclick, '#checklist-'+options.name+' .item', null, Y, options.name);
...
M.gradingform_benchmarks.itemclick = function(e, Y, name) {
    var el = e.target;
    ...
    // recalc the scores
    M.gradingform_benchmarks.recalculatetotals(Y, name);
};

M.gradingform_benchmarks.recalculatetotals = function(Y, name) {
