...
/** benchmarks: Used to compare our gradeitem_type against. */
...
class gradingform_benchmarks_controller extends gradingform_controller {
     ...
     * @return gradingform_benchmarks_renderer
     */
    ...
    protected function delete_plugin_definition() {
        ...
        // delete all fillings
        $DB->delete_records_list('gradingbench_fills', 'instanceid', $instances);
        // delete instances
        $DB->delete_records_list('grading_instances', 'id', $instances);
        // get the list of groups records
        $groups = array_keys($DB->get_records('gradingbench_grp', array('definitionid' => $this->definition->id), '', 'id'));
        // delete checklist items items
        $DB->delete_records_list('gradingbench_items', 'groupid', $groups);
        // delete groups
        $DB->delete_records_list('gradingbench_grp', 'id', $groups);
    }
    ...
    public function get_or_create_instance($instanceid, $raterid, $itemid) {
        ...
        if ($itemid && $raterid) {
            if ($rs = $DB->get_records('grading_instances', array('definitionid' => $this->definition->id, 'raterid' => $raterid, 'itemid' => $itemid), 'timemodified DESC', '*', 0, 1)) {
                ...
                if ($record->status == gradingform_benchmarks_instance::INSTANCE_STATUS_INCOMPLETE &&
    ...
    public function extend_settings_navigation(settings_navigation $settingsnav, navigation_node $node=null) {
        $node->add(get_string('definechecklist', 'gradingform_benchmarks'),
            $this->get_editor_url(), settings_navigation::TYPE_CUSTOM,
            null, null, new pix_icon('icon', '', 'gradingform_benchmarks'));
    }
    ...
    public function extend_navigation(global_navigation $navigation, navigation_node $node=null) {
        ...
        if ($this->is_form_defined() && ($options = $this->get_options()) && !empty($options['alwaysshowdefinition'])) {
            $node->add(get_string('gradingof', 'gradingform_benchmarks', get_grading_manager($this->get_areaid())->get_area_title()),
     ...
     * @param stdClass $newdefinition checklist definition data as coming from gradingform_benchmarks_editchecklist::get_data()
    ...
    public function update_or_check_checklist(stdClass $newdefinition, $usermodified = null, $doupdate = false) {
        ...
        foreach ($newgroups as $id => $group) {
            ...
            if (preg_match('/^NEWID\d+$/', $id)) {
                ...
                if ($doupdate) {
                    $id = $DB->insert_record('gradingbench_grp', $data);
                }
                $haschanges[5] = true;
            } else {
                // update group in DB
                ...
                if (!empty($data)) {
                    ...
                    $data['id'] = $id;
                    if ($doupdate) {
                        $DB->update_record('gradingbench_grp', $data);
                    }
                    $haschanges[1] = true;
                }
                ...
                foreach ($currentgroups[$id]['items'] as $itemid => $currentitem) {
                    ...
                    if (!array_key_exists($itemid, $itemsdata)) {
                        if ($doupdate) {
                            $DB->delete_records('gradingbench_items', array('id' => $itemid));
                        }
                        $haschanges[4] = true;
                    }
                }
            ...
            foreach ($itemsdata as $itemid => $item) {
                ...
                if (preg_match('/^NEWID\d+$/', $itemid)) {
                    ...
                    if ($doupdate) {
                        $itemid = $DB->insert_record('gradingbench_items', $data);
                    }
                    ...
                    if (!empty($data)) {
                        ...
                        $data['id'] = $itemid;
                        if ($doupdate) {
                            $DB->update_record('gradingbench_items', $data);
                        }
                        if (isset($data['score'])) {
                            $haschanges[3] = true;
                        }
        ...
        foreach (array_keys($currentgroups) as $id) {
            if (!array_key_exists($id, $newgroups)) {
                if ($doupdate) {
                    $DB->delete_records('gradingbench_grp', array('id' => $id));
                    $DB->delete_records('gradingbench_items', array('groupid' => $id));
                }
                $haschanges[3] = true;
            }
        }
    ...
    protected function load_definition() {
        ...
        $sql = "SELECT gd.*,
             ...
             LEFT JOIN {gradingbench_grp} clg ON (clg.definitionid = gd.id)
             LEFT JOIN {gradingbench_items} cli ON (cli.groupid = clg.id)
    ...
    public static function sql_search_from_tables($gdid) {
        return " LEFT JOIN {gradingbench_grp} clg ON (clg.definitionid = $gdid)
                 LEFT JOIN {gradingbench_items} cli ON (cli.groupid = clg.id)";
    }
...
class gradingform_benchmarks_instance extends gradingform_instance {
    ...
    public function cancel() {
        ...
        parent::cancel();
        $DB->delete_records('gradingbench_fills', array('instanceid' => $this->get_id()));
    }
    ...
    public function copy($raterid, $itemid) {
        ...
        foreach ($currentgrade['groups'] as $groupid => $group) {
            foreach ($group['items'] as $record) {
                ...
                $DB->insert_record('gradingbench_fills', $params);
            }
        }
        return $instanceid;
    }
    ...
    public function get_checklist_filling($force = false) {
        ...
        if ($this->checklist === null || $force) {
            $records = $DB->get_records('gradingbench_fills', array('instanceid' => $this->get_id()));
        ...
        foreach ($data['groups'] as $groupid => $group) {
            foreach($group['items'] as $itemid => $record) {
                ...
                if (!array_key_exists($groupid, $currentgrade['groups']) || !array_key_exists($itemid, $currentgrade['groups'][$groupid]['items'])) {
                    ...
                    $DB->insert_record('gradingbench_fills', $newrecord);
                } else {
                    ...
                    if (count($newrecord) > 1) {
                        $DB->update_record('gradingbench_fills', $newrecord);
                    }
                }
            }
        }
        ...
        foreach ($currentgrade['groups'] as $groupid => $group) {
            foreach($group['items'] as $itemid => $record) {
                ...
                if (empty($data['groups'][$groupid]['items'][$itemid]['id']) && empty($data['groups'][$groupid]['items'][$itemid]['remark'])) {
                    $DB->delete_records('gradingbench_fills', array('id' => $record['id']));
                }
            }
        }
    ...
    public function render_grading_element($page, $gradingformelement) {
        if (!$gradingformelement->_flagFrozen) {
            $module = array('name'=>'gradingform_benchmarks', 'fullpath'=>'/grade/grading/form/benchmarks/js/checklist.js');
            $page->requires->js_init_call('M.gradingform_benchmarks.init', array(array('name' => $gradingformelement->getName())), true, $module);
            $mode = gradingform_benchmarks_controller::DISPLAY_EVAL;
        } else {
            if ($gradingformelement->_persistantFreeze) {
                $mode = gradingform_benchmarks_controller::DISPLAY_EVAL_FROZEN;
            } else {
        ...
        if ($value === null) {
            ...
            $html .= html_writer::tag('div', get_string('checklistnotcompleted', 'gradingform_benchmarks'), array('class' => 'gradingform_benchmarks-error'));
        }
        $currentinstance = $this->get_current_instance();
        if ($currentinstance && $currentinstance->get_status() == gradingform_instance::INSTANCE_STATUS_NEEDUPDATE) {
            $html .= html_writer::tag('div', get_string('needregrademessage', 'gradingform_benchmarks'), array('class' => 'gradingform_benchmarks-regrade'));
        }
        $haschanges = false;
        if ($currentinstance) {
        ...
        if ($this->get_data('isrestored') && $haschanges) {
            $html .= html_writer::tag('div', get_string('restoredfromdraft', 'gradingform_benchmarks'), array('class' => 'gradingform_benchmarks-restored'));
        }

        $html .= html_writer::tag('div', $this->get_controller()->get_formatted_description(), array('class' => 'gradingform_benchmarks-description clearfix'));
