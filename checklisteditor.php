...
class MoodleQuickForm_checklisteditor extends HTML_QuickForm_input {
     ...
     *
     * @see gradingform_benchmarks_controller::update_or_check_checklist()
    ...
    public function toHtml() {
        global $PAGE;
        $html = $this->_getTabs();
        $renderer = $PAGE->get_renderer('gradingform_benchmarks');
        $data = $this->prepare_data(null, $this->wasvalidated);
        if (!$this->_flagFrozen) {
            $mode = gradingform_benchmarks_controller::DISPLAY_EDIT_FULL;
            $module = array('name'=>'gradingform_benchmarkseditor', 'fullpath'=>'/grade/grading/form/benchmarks/js/checklisteditor.js',
                'strings' => array(array('confirmdeletegroup', 'gradingform_benchmarks'), array('confirmdeleteitem', 'gradingform_benchmarks'),
                    array('groupempty', 'gradingform_benchmarks'), array('itemempty', 'gradingform_benchmarks'), ['maxlengthalert', 'gradingform_benchmarks']
                ));
            $PAGE->requires->js_init_call('M.gradingform_benchmarkseditor.init', array(
    ...
    protected function prepare_data($value = null, $withvalidation = false) {
        ...
        $totalscore = 0;
        $errors = array();
        $return = array('groups' => array(), 'options' => gradingform_benchmarks_controller::get_default_options());
        ...
        if ($withvalidation) {
            if (count($errors)) {
                ...
                foreach ($errors as $error => $v) {
                    $rv[] = get_string($error, 'gradingform_benchmarks');
                }
